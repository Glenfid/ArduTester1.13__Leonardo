...
#elif PROCESSOR_TYP == 32
//################# for m32u4, Arduino Leonardo port F is the Analog input
 ...
 #define TP2 PF6
 #define TP3 PF5
 // Port pin for external Voltage measurement (zener voltage extension) PF4
 #define TPext ((1<<MUX1) | (1<<MUX0))
 // Port pin for 2.5V precision reference used for VCC check (optional) PC4
 #define TPREF (1<<MUX2) 
 #define TPRELAY PF1
 // Port pin for Battery voltage measuring PF0
 #define TPBAT PF0		/* identical to ((1<<MUX2) | (1<<MUX0)) */
...
/* define the input pin for frequency measuring and also the pin change monitoring port for measuring the periode */
 #define FREQINP_DDR DDRB
 #define FREQINP_PIN PB3
 #define PCMSK_FREQ PCMSK0
 #define PCINT_FREQ PCINT3
/* define both input pins for rotaty encoder */
 #ifndef ROTARY_1_DDR
 ...
 #endif
 #ifndef ROTARY_1_PIN
  // default connection is PD0
  #define ROTARY_1_PIN PD0
 #endif
 #ifndef ROTARY_2_PIN
  // PD3 is connected to character LCD D5/D7 or ST7565 RS
  #define ROTARY_2_PIN PD3
...
/* ************************************************************************* */
 ...
 #define ON_PIN PA6      // This Pin is switched to high to switch power on
   ...
   #define ON_PIN PD7      // This Pin is switched to high to switch power on
...
/* ************************************************************************* */
 ...
 // Strip Grid board version
    ...
    #define RST_PIN_REG PINE
	#define RST_PIN PE6     //Pin, is switched to low, if push button is pressed
   // option STRIP_GRID is used to specify other connection of the LCD (chinese version),
   // the RST_PIN remain at PE6.
  #else
   #define RST_PORT PORTD
   #define RST_PIN_REG PIND
   #define RST_PIN PD2     //Pin, is switched to low, if push button is pressed
  #endif
 #else		/* no STRIP_GRID_BOARD */
 // normal layout version and Arduino LEONARDO, changed from PD7 to PC3 for UNO
 	#ifdef ARDUINO_LEONARDO 
   #define RST_PORT PORTF
   #define RST_PIN_REG PINF
   #define RST_PIN PF4     //Pin, is switched to low, if push button is pressed
  #else
   #define RST_PORT PORTE
   #define RST_PIN_REG PINE
   #define RST_PIN PE6     //Pin, is switched to low, if push button is pressed
...
/* ----------------------------------------------------------------------------- */
 ...
 /* The SPI interface uses four signals  RES, EN, RS and B0 */
  /* additional Chip Enable CE is defined */
   ...
   // alternative connection of graphical LCD for the chinese T4 board with PD2 <--> LCD-REST patch
   /* the Reset Pin, 2 = Reset */
   ...
   /* Chip Enable input */
   #define HW_LCD_CE_DDR          DDRC
   #define HW_LCD_CE_PORT         PORTC
   #define HW_LCD_CE_PIN          6
...
/* ----------------------------------------------------------------------------- */
 ...
 /* only two wires are required for the I2C interface: SCL and SDA */
  ...
  #define LCD_I2C_ADDR 0x3C		/* SSD1306 controller defines 0x3c or 0x3d (SA0=1) as address */
   ...
   #define HW_LCD_SCL_PIN          6

   #define HW_LCD_SDA_PORT         PORTD
   #define HW_LCD_SDA_PIN          1
...
/* ----------------------------------------------------------------------------- */
 ...
 #elif PROCESSOR_TYP == 32
   #define HW_LCD_EN_PORT         PORTC
   #define HW_LCD_EN_DDR          DDRC
   #define HW_LCD_EN_PIN          6
...
/* ----------------------------------------------------------------------------- */
 ...
 // normal connection for the 4-bit parallel interface and ATmega324/644/1284
   ...
   #define HW_LCD_RS_PORT         PORTE
   #define HW_LCD_RS_PIN          6
 
   #define HW_LCD_B4_PORT         PORTB
   #define HW_LCD_B4_PIN          3
   #define HW_LCD_B5_PORT         PORTD
   #define HW_LCD_B5_PIN          0
   ...
   #ifdef ARDUINO_LEONARDO
    #define HW_LCD_EN_PORT         PORTD
    #define HW_LCD_EN_PIN          7

    #define HW_LCD_RS_PORT         PORTE
    #define HW_LCD_RS_PIN          6

    #define HW_LCD_B4_PORT         PORTC
    #define HW_LCD_B4_PIN          6
    #define HW_LCD_B5_PORT         PORTB
    #define HW_LCD_B5_PIN          3
    #define HW_LCD_B6_PORT         PORTD
    #define HW_LCD_B6_PIN          0
    #define HW_LCD_B7_PORT         PORTD
    #define HW_LCD_B7_PIN          1
   #else
    #define HW_LCD_EN_PORT         PORTC
...
//#define SWUART_INVERT
 ...
 #define TXD_MSK 0	/* no output for ADC port */
#elif PROCESSOR_TYP == 32
 #define SERIAL_PORT PORTF
 #define SERIAL_DDR DDRF
 #define SERIAL_BIT PF4	//TxD-Pin of Software-UART; must be at Port F !
 #ifdef WITH_UART
  ...
  #endif
 #else	/* no WITH_UART */
 // If you use any pin of port F for output, you should define all used pins in TXD_MSK.
 // With TXD_VAL you can specify the default level of output pins for all port F output pins.
  #define TXD_MSK 0
  #define TXD_VAL 0
 #endif	/* WITH_UART */
#else
