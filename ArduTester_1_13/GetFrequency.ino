...
//=================================================================
// measure frequency at external pin T0 (PD4@mega328 or PD7@mega644) with Counter 0
...
#define FREQ_DIV 16
 ...
 /* Pin change interrupt is PCINT8 (PD7) */
...
void GetFrequency(uint8_t range) {
 ...
 if (range == 0) {  
  ...
  for (mm=0;mm<240;mm++) {
     ...
     // *************************************************************************
     //set up Counter 0
     // Counter 0 is used to count the external signal connected to T0 (PD4 or PD7)
 ...
 #define CNT1_DIVIDER ((1<<CS12) | (1<<CS10))
  ...
  #warning F_CPU can not be divided by 256, measured frequency is wrong!
	...
	EICRB = (0<<ISC61) | (1<<ISC60); // set int6 pin change
        ...
        sei();
        PCMSK_FREQ |= (1<<PCINT_FREQ);	// monitor PD4 PCINT20 or PD7 PCINT8 pin change
        for (ii=0;ii<250;ii++) {
           wait20ms();
           wdt_reset();
           if (!(RST_PIN_REG & (1<<RST_PIN))) taste = 1;	// user request stop of operation
        ...
        PCMSK_FREQ &= ~(1<<PCINT_FREQ);		// stop monitor PD4 PCINT20 or PD7 PCINT8 pin change
...
ISR(PCINTx_vect, ISR_BLOCK)
{
  ...
  if (pinchange_count >= pinchange_max) {
     ...
     TCCR0B = 0;		// stop counter 0
     PCMSK_FREQ &= ~(1<<PCINT_FREQ);	// disable monitoring of PD4 PCINT20 or PD7 PCINT8 pin change
#endif
//     PCICR &= ~(1<<PCI_ENABLE_BIT);	// disable the interrupt
  }
  pinchange_count++;
